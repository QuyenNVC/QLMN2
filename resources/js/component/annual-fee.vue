    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Quản lý danh mục học phí hàng năm</h4>
  ...
  methods: {
    ...
    async fetchData() {
      await axios
        ...
        .then((response) => {
          if (response.data.status == true) {
            this.tableData = response.data.annualfees;
    ...
    handleCreate() {
      if (this.validateForm()) {
        if (this.isShowCreateTitle) {
          if (!this.checkExist()) {
            return axios
              .post("createAnnualFee", this.form)
              .then((response) => {
                if (response.data.status == true) {
                  this.isErrorForm = false;
                  this.titleMessage = "";
          ...
          if (!this.checkExist()) {
            return axios
              .post("updateAnnualFee", this.form)
              .then((response) => {
                if (response.data.status == true) {
                  this.isErrorForm = false;
                  this.titleMessage = "";
