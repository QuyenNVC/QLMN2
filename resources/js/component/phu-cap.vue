            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group-input">
                  <span class="label-input form-label required">Tên phụ cấp: </span>
                  <el-input
                ...
                <div class="form-group-input">
                  <span class="label-input">Phụ cấp: </span>
                  <el-input
                    placeholder="Số phụ cấp"
                    v-model="form.phu_cap"
                  ></el-input>
                </div>
                
                <div class="form-group-input">
                  <span class="label-input">Đơn vị tính: </span>
                  <el-select v-model="form.don_vi_tinh" placeholder="Chọn đơn vị tính">
                    <el-option
                      label="VND"
                      value="vnd">
                    </el-option>
            ...
            >
              ...
              <el-table-column label="Phụ cấp"> 
                <template slot-scope="scope">
                  {{ scope.row.phu_cap | formatMoney }}
                </template>  
              </el-table-column>
              <el-table-column label="Đơn vị tính" prop="don_vi_tinh"> </el-table-column>
              <el-table-column label="Ghi chú" prop="ghi_chu"> </el-table-column>
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        .then(async () => {
          await axios.post("deletePhuCap/" + row.id).then((response) => {
            console.log(response);
            if (response.data.status == true) {
              this.$notify({
                title: "Thông báo",
                message: `Xóa phụ cấp "${username}" thành công!`,
                type: "success",
              });
              this.fetchData();
            } else {
    ...
    async handleCreate() {
      if (this.validateForm()) {
        this.form.phu_cap = this.form.phu_cap.replace(',', '');
        if ( this.isCreate ) {
