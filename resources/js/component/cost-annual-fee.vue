    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Quản lý định mức học phí hàng năm</h4>
        ...
        <form-wrapper>
          ...
          <template>
            <div class="row">
              ...
              <div class="col-md-6" v-for="item in annualFees" :key="item.id">
                <div class="form-group-input">
...
export default {
  ...
  data: function () {
    return {
      ...
      title: "Thêm định mức học phí hàng năm",
      gradeUpdate: "",
      yearUpdate: "",
      filter: {
        id_coso: "",
  ...
  methods: {
    handleEdit(index, row) {
      ...
      for (let j = 0; j < this.annualFees.length; j++) {
        this.form[this.annualFees[j].name] = row[this.annualFees[j].name]
          ? row[this.annualFees[j].name].replace(/,/g, "")
          : "";
      }
      this.isShowCreateTitle = false;
      this.title = `Cập nhật định mức học phí hàng năm của khối lớp ${row.grade_name} năm học ${row.school_year_name}`;
    },
    fetchData() {
      axios
        .get("/getCostAnnualFee/" + this.filter.id_coso)
        .then((response) => {
          this.tableData = response.data.costAnnualFees;
          for (let i = 0; i < this.tableData.length; i++) {
            ...
            if (this.tableData[i].fees) {
              for (let j = 0; j < this.annualFees.length; j++) {
                ...
                this.tableData[i][this.annualFees[j].name] = this.tableData[i]
                  .fees[j]
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.status == true) {
            this.annualFees = response.data.annualfees;
            for (let i = 0; i < this.annualFees.length; i++) {
              var fee = this.annualFees[i].name;
              this.form[fee] = "";
            }
    ...
    resetForm() {
      ...
      for (let i = 0; i < this.annualFees.length; i++) {
        this.form[this.annualFees[i].name] = "";
      }
      this.form.fees = [];
      this.isShowCreateTitle = true;
      this.title = "Thêm định mức học phí hàng năm";
      this.yearUpdate = "";
      this.gradeUpdate = "";
    },
    formatBeforeSend() {
      this.form.fees = [];
      for (var i = 0; i < this.annualFees.length; i++) {
        var item = this.annualFees[i];
      ...
      if (this.validateForm()) {
        if (this.isShowCreateTitle) {
          if (!this.checkExist()) {
            ...
            return axios
              .post("createCostAnnualFee", this.form)
              .then((response) => {
                if (response.data.status) {
                  ...
                  this.$notify({
                    title: "Thông báo",
                    message: `Tạo thành công định mức học phí hàng năm cho khối lớp ${grade} năm học ${schoolYear}!`,
                    type: "success",
                  });
                  return true;
                } else {
          ...
          if (!this.checkExist()) {
            ...
            console.log(this.form);
            return axios
              .post("updateCostAnnualFee", this.form)
              .then((response) => {
                if (response.data.status) {
                  this.fetchData();
                  this.$notify({
                    title: "Thông báo",
                    message: `Cập nhật thành công định mức học phí hàng năm cho khối lớp ${this.gradeUpdate}, năm học ${this.yearUpdate}!`,
                    type: "success",
                  });
                  return true;
                } else {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        await axios
          .post("deleteCostAnnualFee/" + row.id)
          .then((response) => {
            if (response.data.status == true) {
              this.$notify({
                title: "Thông báo",
                message: `Xóa loại định mức học phí hàng năm thành công!`,
                type: "success",
              });
              this.fetchData();
            } else {
