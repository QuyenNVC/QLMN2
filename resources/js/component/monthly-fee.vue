    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Quản lý danh mục học phí hàng tháng</h4>
...
export default {
  ...
  data() {
    return {
      tableData: [],
      search: "",
      title: "Thêm mới danh mục học phí",
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.title = `Cập nhật danh mục học phí "${row.name}"`;
    },
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        .then(async () => {
          await axios
            .post("deleteMonthlyFee/" + row.id)
            .then((response) => {
              if (response.data.status == true) {
                this.$notify({
                  title: "Thông báo",
                  message: `Xóa danh mục học phí "${name}" thành công!`,
                  type: "success",
                });
                this.fetchData();
              } else {
    ...
    async fetchData() {
      await axios
        ...
        .then((response) => {
          if (response.data.status == true) {
            this.tableData = response.data.monthlyfees;
    ...
    handleCreate() {
      if (this.validateForm()) {
        if (this.isShowCreateTitle) {
          if (!this.checkExist()) {
            return axios
              .post("createMonthlyFee", this.form)
              .then((response) => {
                if (response.data.status == true) {
                  this.isErrorForm = false;
                  this.titleMessage = "";
                  ...
                  this.$notify({
                    title: "Thông báo",
                    message: `Thêm mới danh mục học phí "${this.form.name}" thành công!`,
                    type: "success",
                  });
                  return true;
                } else {
            ...
            this.$notify({
              title: "Thông báo",
              message: `Danh mục học phí này đã tồn tại`,
              type: "warning",
            });
          }
        } else {
          if (!this.checkExist()) {
            return axios
              .post("updateMonthlyFee", this.form)
              .then((response) => {
                if (response.data.status == true) {
                  this.isErrorForm = false;
                  this.titleMessage = "";
                  ...
                  this.$notify({
                    title: "Thông báo",
                    message: `Cập nhật danh mục học phí "${this.form.name}" thành công!`,
                    type: "success",
                  });
                  return true;
                } else {
    ...
    validateForm() {
      if (this.form.name == "") {
        this.$notify({
          title: "Thông báo",
          message: "Tên danh mục học phí không được để trống",
          type: "warning",
        });
        return false;
      }
