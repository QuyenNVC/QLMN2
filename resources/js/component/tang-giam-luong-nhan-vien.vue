    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Quản lý tăng/giảm lương nhân viên</h4>
                  ...
                  >
                    <div class="form-group-input">
                      <el-checkbox
                        ...
                        @change="changeCheck()"
                      >
                        {{ item.name }} ( {{ item.tang_giam | formatMoney
                        }}{{ item.don_vi_tinh }} )
                      </el-checkbox>
                    </div>
                  </div>
...
export default {
  name: "tang-giam-luong-nhan-vien",
  mixins: [mixin],
  props: {
    phongBanUser: {
      type: String,
  ...
  methods: {
    ...
    async getAllNhanVienDangLamViec() {
      ...
      await axios
        .get(
          `dataNhanVienDangLamViecTangGiamLuong/${this.month}/${this.year}/${this.phongBan}`,
          {
            params: {
              id_coso: this.filter.id_coso,
            },
    ...
    async handleCreate() {
      if (this.validateForm()) {
        await axios
          .post("updateTangGiamLuongNhanVien", {
            thang: this.month,
            nam: this.year,
            phuCap: this.form,
          })
          .then((response) => {
            if (response.data.status == true) {
              ...
              this.$notify({
                title: "Thông báo",
                message: `Cập nhật cho nhân viên "${this.form.name}" thành công!`,
                type: "success",
              });
              this.resetForm();
    ...
    async getAllPhuCap() {
      await axios.get("dataTangGiamLuong").then((response) => {
        if (response.data.status == true) {
          this.phuCaps = response.data.tang_giam_luong;
      ...
      for (const [key, value] of Object.entries(data)) {
        if (value == true) {
          this.phuCaps.forEach((item) => {
            if (key == item.id) {
              if (str == "") {
                str += item.name + `: ${this.formatMoney(item.tang_giam)}`;
              } else {
                str +=
                  ", " + item.name + `: ${this.formatMoney(item.tang_giam)}`;
              }
            }
          });
        }
