    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Báo cáo ngày công nhân viên</h4>
          ...
          <div class="wrap-form">
            <table>
              <tr>
                ...
                <th class="ho-ten">Họ Tên</th>
                <th v-for="item in hinhThucChamCongs" :key="item.id">
                  {{ item.name }}
                </th>
              </tr>
              <tr v-for="(item, index) in nhanViens" :key="item.index">
                <td>{{ index + 1 }}</td>
                <td>{{ item.ma_nv }}</td>
                <td>{{ item.fullname }}</td>
                <td
                ...
                >
                  <span
                    v-if="getNgayCongOfNhanVienByDate(item.ma_nv).length == 0"
                  >
                    0
                  </span>
                  <span
...
export default {
  name: "bao-cao-ngay-cong-nhan-vien",
  components: {
    VTreeview,
  },
  directives: { mask },
  ...
  methods: {
    ...
    async getAllNhanVienPhongBan() {
      await axios.get("dataNhanVien/" + this.phongBan).then((response) => {
        if (response.data.status == true) {
