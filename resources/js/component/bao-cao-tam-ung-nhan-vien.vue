    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Danh sách nhân viên có tạm ứng lương tháng {{ month }} năm {{ year }}</h4>
...
export default {
  name: "bao-cao-tam-ung-nhan-vien",
  mixins: [mixin],
  data() {
    return {
      tableData: [],
  ...
  methods: {
    ...
    async getAllNhanVienDangLamViec() {
      console.log(`dataNhanVienDangLamViecUngLuongBaoCao/${this.month}/${this.year}/${this.phongBan}`);
      await axios.get(`dataNhanVienDangLamViecUngLuongBaoCao/${this.month}/${this.year}/${this.phongBan}`).then((response) => {
        if (response.data.status == true) {
