    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4 class="page-title">Quản lý định mức học phí tháng</h4>
        ...
        <form-wrapper>
          ...
          <template>
            <div class="row">
              ...
              <div
                class="col-md-6"
                v-for="monthlyFee in monthlyFees"
                :key="monthlyFee.id"
              >
                <div class="form-group-input">
                  ...
                  <input
                    type="text"
                    v-model="form[monthlyFee.name]"
                    v-money="money"
                    class="el-input__inner el-input"
                    :placeholder="monthlyFee.name"
                  />
                </div>
              </div>
              <div class="col-md-6">
        ...
        <div class="wrap-table">
          ...
          >
            ...
            <el-table-column
              v-for="monthlyFee in monthlyFees"
              :key="monthlyFee.id"
              :label="monthlyFee.name"
              :prop="monthlyFee.name"
              header-align="center"
              align="right"
              width="150"
            ></el-table-column>
...
export default {
  ...
  data: function () {
    return {
      ...
      title: "Thêm định mức học phí tháng",
      gradeUpdate: "",
      yearUpdate: "",
      filter: {
        id_coso: "",
  ...
  methods: {
    handleEdit(index, row) {
      ...
      for (let j = 0; j < this.monthlyFees.length; j++) {
        this.form[this.monthlyFees[j].name] = row[this.monthlyFees[j].name]
          ? row[this.monthlyFees[j].name].replace(/,/g, "")
          : "";
      }
      this.isShowCreateTitle = false;
      this.title = `Cập nhật định mức học phí tháng của khối lớp ${row.grade_name} năm học ${row.school_year_name}`;
    },
    fetchData() {
      axios
        .get("/getCostMonthlyFee/" + this.filter.id_coso)
        .then((response) => {
          this.tableData = response.data.costMonthlyFees;
          for (let i = 0; i < this.tableData.length; i++) {
            ...
            if (this.tableData[i].fees) {
              for (let j = 0; j < this.monthlyFees.length; j++) {
                ...
                this.tableData[i][this.monthlyFees[j].name] = this.tableData[i]
                  .fees[j]
      ...
      await axios
        ...
        .then((response) => {
          if (response.data.status == true) {
            this.monthlyFees = response.data.monthlyfees;
            for (let i = 0; i < this.monthlyFees.length; i++) {
              var monthly_fee = this.monthlyFees[i].name;
              this.form[monthly_fee] = "";
            }
          }
        });
    },
    resetForm() {
      ...
      for (let i = 0; i < this.monthlyFees.length; i++) {
        this.form[this.monthlyFees[i].name] = "";
      }
      this.form.fees = [];
      this.isShowCreateTitle = true;
      this.title = "Thêm định mức học phí tháng";
      this.yearUpdate = "";
      this.gradeUpdate = "";
    },
    formatBeforeSend() {
      this.form.fees = [];
      for (var i = 0; i < this.monthlyFees.length; i++) {
        var monthlyFee = this.monthlyFees[i];
        this.form.effective_date = this.form.effective_date
          ? this.formatDate(this.form.effective_date)
          : "";
        this.form.fees.push({
          id: monthlyFee.id,
          value: this.form[monthlyFee.name].replace(/,/g, ""),
        });
      }
    },
    formatDate(date) {
      ...
      if (this.validateForm()) {
        if (this.isShowCreateTitle) {
          if (!this.checkExist()) {
            ...
            return axios
              .post("createCostMonthlyFee", this.form)
              .then((response) => {
                if (response.data.status) {
                  ...
                  this.$notify({
                    title: "Thông báo",
                    message: `Tạo thành công định mức học phí tháng cho khối lớp ${grade} năm học ${schoolYear}!`,
                    type: "success",
                  });
                  return true;
                } else {
            ...
            this.$notify({
              title: "Thông báo",
              message: `Định mức học phí này đã tồn tại`,
              type: "warning",
            });
          }
        } else {
          if (!this.checkExist()) {
            this.formatBeforeSend();
            return axios
              .post("updateCostMonthlyFee", this.form)
              .then((response) => {
                if (response.data.status) {
                  this.fetchData();
                  this.$notify({
                    title: "Thông báo",
                    message: `Cập nhật thành công định mức học phí tháng cho khối lớp ${this.gradeUpdate}, năm học ${this.yearUpdate}!`,
                    type: "success",
                  });
                  return true;
                } else {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        await axios
          .post("deleteCostMonthlyFee/" + row.id)
          .then((response) => {
            if (response.data.status == true) {
              this.$notify({
                title: "Thông báo",
                message: `Xóa loại định mức học phí tháng thành công!`,
                type: "success",
              });
              this.fetchData();
            } else {
    ...
    checkExist() {
      ...
      checkName = this.tableData.find(
        (costmonthlyfee) =>
          costmonthlyfee.school_year == this.form.school_year &&
          costmonthlyfee.grade == this.form.grade_id &&
          costmonthlyfee.id != this.form.id
      );
