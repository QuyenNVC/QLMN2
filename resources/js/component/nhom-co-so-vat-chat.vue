    ...
    <div class="page-breadcrumb">
      <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
          <h4>Quản lý nhóm cơ sở vật chất</h4>
          ...
          <div class="col-12">
            <div class="wrap-table table-record">
              ...
              >
                ...
                <el-table-column label="Tên nhóm cơ sở vật chất" prop="name">
                </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      arraySelectedRecord: [],
      resetCheckbox: 0,
      title: "Thêm nhóm cơ sở vật chất",
      form: {
        id: "",
        name: "",
        note: "",
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        .then(async () => {
          await axios
            .post("/deleteNhomCoSoVatChat", { array: [row.id] })
            .then((response) => {
              if (response.data.status == true) {
                this.$notify({
                  title: "Thông báo",
                  message: `Xóa nhóm cơ sở vật chất "${name}" thành công!`,
                  type: "success",
                });
                this.fetchData();
              } else {
    ...
    validateForm() {
      if (this.form.name == "") {
        this.$notify({
          title: "Cảnh báo",
          message: "Tên nhóm cơ sơ vật chất không được bỏ trống!",
          type: "warning",
        });
        return false;
      }
    ...
    resetForm() {
      this.title = "Thêm nhóm cơ sở vật chất";
      this.form.name = "";
      this.form.id = "";
      this.form.note = "";
    },
    handleEdit(row) {
      this.title = `Cập nhật nhóm cơ sở vật chất "${row.name}"`;
    ...
    handleSubmit() {
      if (this.validateForm()) {
        if (!this.checkExist()) {
          return axios
            .post("submitNhomCoSoVatChat", this.form)
            .then((response) => {
              if (response.data.status) {
                ...
                this.$notify({
                  ...
                  message: !this.form.id
                    ? `Thêm nhóm cơ sở vật chất "${this.form.name}" thành công!`
                    : `Cập nhật nhóm cơ sở vật chất "${this.form.name}" thành công!`,
                  type: "success",
                });
                this.resetForm();
                return true;
          ...
          this.$notify({
            title: "Thông báo",
            message: "Nhóm cơ sở vật chất này đã tồn tại",
            type: "warning",
          });
        }
      }
