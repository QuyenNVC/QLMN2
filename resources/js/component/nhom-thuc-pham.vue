            ...
            <div class="row">
              ...
              <div class="col-12">
                <div class="form-group-input">
                  ...
                  <el-input
                    placeholder="Tên nhóm thực phẩm"
                    v-model="form.name"
                  ></el-input>
                </div>
              </div>
              <div class="col-12">
                <div class="form-group-input">
                  <span class="label-input" style="margin-right: 48px"
                    >Ghi chú:
                  </span>
                  <el-input
                    type="textarea"
          ...
          <div class="col-12">
            <div class="wrap-table table-record">
              ...
              >
                ...
                ></el-table-column>
                <el-table-column
                  label="Mã nhóm thực phẩm"
                  prop="ma_nhom"
                  header-align="center"
                >
                </el-table-column>
                <el-table-column
                  label="Tên nhóm thực phẩm"
                  prop="name"
                  header-align="center"
                >
                </el-table-column>
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("Bạn thật sự muốn xóa dữ liệu này?", "Cảnh báo", {
        ...
        .then(async () => {
          await axios
            .post("/deleteNhomThucPham", { array: [row.id] })
            .then((response) => {
              if (response.data.status == true) {
                this.$notify({
                  title: "Thông báo",
                  message: `Xóa nhóm thực phẩm "${name}" thành công!`,
                  type: "success",
                });
                this.fetchData();
              } else {
    ...
    // phần xử lý form
    validateForm() {
      if (formatString(this.form.ma_nhom) == "") {
        this.$notify({
          title: "Cảnh báo",
          message: "Mã nhóm thực phẩm không được bỏ trống!",
          type: "warning",
        });
        return false;
      }
      if (formatString(this.form.ma_nhom).length > 3) {
        this.$notify({
          title: "Cảnh báo",
          message: "Mã nhóm thực phẩm không được nhiều hơn 3 ký tự",
          type: "warning",
        });
        return false;
      }
      if (formatString(this.form.name) == "") {
        this.$notify({
          title: "Cảnh báo",
          message: "Tên nhóm thực phẩm không được bỏ trống!",
          type: "warning",
        });
        return false;
      }
      ...
      checkName = this.tableData.find(
        (item) =>
          (formatString(item.name) == formatString(this.form.name) ||
            formatString(item.ma_nhom) == formatString(this.form.ma_nhom)) &&
          item.id != this.form.id
      );
      return checkName ? true : false;
    },
    resetForm() {
      this.title = "Thêm nhóm thực phẩm";
      for (let key in this.form) {
        this.form[key] = "";
      }
    },
    handleEdit(row) {
      this.title = `Cập nhật nhóm thực phẩm "${row.name}"`;
      for (let key in this.form) {
    ...
    handleSubmit() {
      if (this.validateForm()) {
        if (!this.checkExist()) {
          return axios
            .post("submitNhomThucPham", this.form)
            .then((response) => {
              if (response.data.status) {
                ...
                this.$notify({
                  ...
                  message: !this.form.id
                    ? `Thêm nhóm thực phẩm "${this.form.name}" thành công!`
                    : `Cập nhật nhóm thực phẩm "${this.form.name}" thành công!`,
                  type: "success",
                });
                this.resetForm();
                return true;
          ...
          this.$notify({
            title: "Thông báo",
            message: "Nhóm thực phẩm này đã tồn tại",
            type: "warning",
          });
        }
      }
